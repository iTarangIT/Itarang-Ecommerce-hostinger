...
const PALETTES = {
  1: { body: '#152a4d', bodyDark: '#0d1c35', face: '#1d3868', line: '#3b5c99', text: '#e8eefb' },
  2: { body: '#e6ebf4', bodyDark: '#cdd7e6', face: '#f4f7fb', line: '#8ea3c4', text: '#16294a' },
  3: { body: '#0f2340', bodyDark: '#091729', face: '#17325c', line: '#2f4f88', text: '#e8eefb' },
};

const AMBER = '#faa113';
const AMBER_DEEP = '#d1810a';
const BG = '#f3f6fb';
const BG_EDGE = '#e4eaf4';
const SHADOW = '#0f2340';
...
const wordmark = (x, y, colour, size = 22) =>
  `<text x="${x}" y="${y}" font-family="Sora, system-ui, sans-serif" font-size="${size}" font-weight="700" letter-spacing="1.5" fill="${colour}" opacity="0.85">iTarang</text>`;

const caption = (text, colour) =>
  `<text x="${W / 2}" y="742" text-anchor="middle" font-family="Inter, system-ui, sans-serif" font-size="22" font-weight="500" letter-spacing="0.6" fill="${colour}" opacity="0.5">${text}</text>`;
...
function inverterFront(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="272" y="240" width="150" height="16" rx="8" fill="${AMBER}" opacity="0.9"/>
    ...
    <rect x="470" y="556" width="58" height="20" rx="10" fill="url(#amber)"/>
...
function inverterAngle(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="302" y="262" width="110" height="14" rx="7" fill="${AMBER}"/>
    <rect x="302" y="290" width="150" height="9" rx="4.5" fill="${p.line}" opacity="0.6"/>
    ${grille(292, 386, 190, 8, p.line)}
    ${led(462, 262, AMBER, 6)}
    ${wordmark(296, 566, p.text, 20)}
    <path d="M540 210 L600 232 L600 262 L540 240 Z" fill="${AMBER}" opacity="0.75"/>
  </g>`;
}
...
function inverterDetail(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="222" y="278" width="230" height="26" rx="13" fill="${AMBER}"/>
    ...
    <rect x="482" y="462" width="128" height="58" rx="14" fill="url(#amber)" opacity="0.9"/>
  </g>`;
}
...
function inverterPanel(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="486" y="320" width="78" height="8" rx="4" fill="${AMBER}"/>
    ...
    ${led(258, 486, AMBER, 8)}${led(300, 486, '#3fbf7f', 8)}${led(342, 486, p.line, 8)}
  </g>`;
}
...
function boxArt(p, label) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="264" y="368" width="104" height="14" rx="7" fill="${AMBER}" opacity="0.9"/>
    <rect x="264" y="400" width="80" height="10" rx="5" fill="${p.text}" opacity="0.4"/>
    <text x="440" y="416" font-family="Sora, system-ui, sans-serif" font-size="24" font-weight="700" fill="${p.text}" opacity="0.75">${label}</text>
...
function batteryFront(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="236" y="278" width="200" height="18" rx="9" fill="${AMBER}"/>
    ...
    <rect x="420" y="530" width="144" height="34" rx="10" fill="url(#amber)" opacity="0.85"/>
  </g>`;
}
...
function batteryAngle(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="272" y="300" width="150" height="16" rx="8" fill="${AMBER}"/>
...
function batteryDetail(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="220" y="450" width="360" height="16" rx="8" fill="${AMBER}" opacity="0.9"/>
...
function batteryPanel(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="236" y="280" width="180" height="16" rx="8" fill="${AMBER}"/>
    ${[0, 1, 2].map((i) => `<g><rect x="216" y="${344 + i * 64}" width="368" height="48" rx="12" fill="${p.bodyDark}" opacity="0.5"/><rect x="238" y="${362 + i * 64}" width="${230 - i * 44}" height="12" rx="6" fill="${p.line}" opacity="0.7"/><rect x="510" y="${362 + i * 64}" width="52" height="12" rx="6" fill="${AMBER}" opacity="${0.85 - i * 0.2}"/></g>`).join('')}
  </g>`;
}
...
function upsFront(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="328" y="212" width="90" height="18" rx="9" fill="${AMBER}"/>
    ...
    <path d="M400 314 L400 340" stroke="${AMBER}" stroke-width="6" stroke-linecap="round"/>
    <circle cx="400" cy="330" r="16" fill="none" stroke="${AMBER}" stroke-width="4" stroke-dasharray="60 30" transform="rotate(-90 400 330)"/>
...
function upsAngle(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="334" y="248" width="66" height="14" rx="7" fill="${AMBER}"/>
    <circle cx="378" cy="366" r="24" fill="${p.bodyDark}"/>
    <path d="M378 352 L378 374" stroke="${AMBER}" stroke-width="5" stroke-linecap="round"/>
    ${grille(320, 424, 122, 5, p.line, 20)}
    ${wordmark(322, 566, p.text, 16)}
    <path d="M492 236 L552 262 L552 292 L492 266 Z" fill="${AMBER}" opacity="0.7"/>
  </g>`;
}
...
function upsDetail(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="240" y="290" width="150" height="26" rx="13" fill="${AMBER}"/>
...
function upsPanel(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="196" y="486" width="180" height="14" rx="7" fill="${AMBER}" opacity="0.85"/>
...
function comboFront(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="192" y="262" width="88" height="14" rx="7" fill="${AMBER}"/>
    ...
    <rect x="462" y="338" width="150" height="15" rx="7.5" fill="${AMBER}"/>
    ...
    <path d="M410 400 L430 400" stroke="${AMBER}" stroke-width="8" stroke-linecap="round"/>
    <path d="M398 372 C 420 372 420 428 442 428" stroke="${AMBER}" stroke-width="7" fill="none" stroke-linecap="round" opacity="0.85"/>
  </g>`;
}
...
function comboAngle(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="218" y="288" width="62" height="12" rx="6" fill="${AMBER}"/>
    ...
    <rect x="468" y="358" width="112" height="13" rx="6.5" fill="${AMBER}"/>
    ${grille(468, 408, 122, 4, p.line, 22)}
    <path d="M362 400 C 396 400 400 436 438 436" stroke="${AMBER}" stroke-width="7" fill="none" stroke-linecap="round"/>
  </g>`;
}
...
function comboDetail(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="220" y="304" width="120" height="22" rx="11" fill="${AMBER}"/>
    ...
    <path d="M516 300 L488 336 L508 336 L502 366 L532 328 L512 328 Z" fill="${AMBER}"/>
    <rect x="192" y="470" width="180" height="16" rx="8" fill="${AMBER}" opacity="0.9"/>
...
function comboPanel(p) {
  ...
  <g filter="url(#soft)">
    ...
    <rect x="230" y="296" width="150" height="18" rx="9" fill="${AMBER}"/>
    <rect x="440" y="296" width="130" height="18" rx="9" fill="${p.line}" opacity="0.6"/>
    ${[0, 1, 2].map((i) => `<rect x="206" y="${374 + i * 66}" width="388" height="52" rx="14" fill="${p.bodyDark}" opacity="${0.55 - i * 0.12}"/><circle cx="${240}" cy="${400 + i * 66}" r="12" fill="${AMBER}" opacity="${0.9 - i * 0.25}"/><rect x="272" y="${393 + i * 66}" width="${220 - i * 40}" height="14" rx="7" fill="${p.line}" opacity="0.6"/>`).join('')}
  </g>`;
}
